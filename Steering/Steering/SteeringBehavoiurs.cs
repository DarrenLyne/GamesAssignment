    ...
    {        
        ...
        {           
            ...
            foreach (Cell cell in cells)
            {
                ...
                {
                    List<Entity> entities = cell.Entities;
        ...
        public Vector3 cohesion()
        {
            ...
            if (taggedCount > 0)
            {
                ...
                if (centreOfMass.LengthSquared() == 0)
                {
        ...
        public enum behaviour_type
        {
            ...
            random_walk = 0x40000,
            EnforceNonPenetrationConstraint = 0x80000,
        };
        ...
        private Vector3 calculateWeightedPrioritised()
        {
            ...
            if(isOn(behaviour_type.EnforceNonPenetrationConstraint))
            {
                force = EnforceNonPenetrationConstraint(fighter) * Params.GetWeight("EnforceNonPenetrationConstraintweight");
                if (!accumulateForce(ref steeringForce, force))
                {
                    return steeringForce;
                }
        ...
        private Vector3 EnforceNonPenetrationConstraint(Entity current)
        {
            ...
            foreach (Fighter child in XNAGame.Instance().Children.Where(x=> x.GetType()==typeof(AIFighter)))
            {
                ...
                if (child != current && child.ModelName == "ReaperSovFBX")
                {
                    //calculate the distance between the positions of the entities
                    Vector3 toEntity = current.pos - child.pos;
                    float distfromEachOther = toEntity.Length();

                    //if this distance is smaller than the sum of their radii then this
                    //entity must be moved away in the direction parallel to the
                    //ToEntity vector
                    float amountofOverlap = 100000 - distfromEachOther;
                    if (amountofOverlap >= 0)
                    {
                        //move the entity a distance away equivalent to the amount of overlap.
                        current.force += Vector3.Multiply((Vector3.Divide(toEntity, distfromEachOther)), amountofOverlap);
                    }
